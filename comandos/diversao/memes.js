    ...
    async executar(iCmd, opcoes) {
        switch (opcoes.subComando) {
            case "adicionar": {
                ...
                if (meme) return client.responder(iCmd, "bloqueado", "Esse meme já existe", `Já tem um meme com esse nome, se você deseja editar um meme use \`/${this.nome} editar\``);

                const regex = /^https?:\/\/(?:[a-z0-9-]+\.)+[a-z]{2,6}(?:\/[^/#?]+)+(\.|-)(?:jpg|jpeg|png|gif|webm|webp|mp4|wav|mp3|ogg)/;
                if (!regex.test(opcoes.adicionar.link)) return client.responder(iCmd, "bloqueado", "Arquivo ou link inválido", "Arquivos validos: `jpg, jpeg, png, gif, webm, webp, mp4, wav, mp3, ogg`")
