...
module.exports = {
    ...
    async executar(iCmd, opcoes) {
        ...
        if (usuario.bot) return client.responder(iCmd, "bloqueado", "Bots não tem perfis", "Porque eles teriam um?")
        ...
        const aniversario = perfil.aniversario ? `<t:${proximoAniversario(nascimento).getTime() / 1000}:R>` : "???";
        ...
        const conjuge = relacionamento.conjugeId ? (`<@${relacionamento.conjugeId}>`)
            + ` <t:${Math.round(new Date(relacionamento.dataCasamento).getTime() / 1000)}:R>` : "Ninguém";
