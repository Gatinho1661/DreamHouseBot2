...
module.exports = {
  ...
  exemplos: [
    ...
    { comando: "snipe [número]", texto: "Mostra uma mensagem editada específica do canal" },
    { comando: "snipe [canal]", texto: "Mostra a ultima mensagem editada de um canal" },
    { comando: "snipe [número] [canal]", texto: "Mostra uma mensagem editada específica de um canal" }
  ],
  args: "{numero}, {canal}",
  opcoes: [
    {
  ...
  async executar(iCmd, opcoes) {
    ...
    if (!editSnipes?.length > 0) {
      return client.responder(
        iCmd,
        "bloqueado",
        "Nenhuma mensagem editada encontrada",
        `Se isso persistir fale com o ${client.application.owner.toString()} para arrumar isso`
      );
    }

    const editSnipedmsg = editSnipes[opcoes.numero ? opcoes.numero - 1 : 0];
    if (!editSnipedmsg && opcoes?.numero) {
      return client.responder(
        iCmd,
        "bloqueado",
        "Editsnipe escolhido não encontrado",
        `Escolha um editsnipe entre 1 e ${editSnipes.length}`
      );
    }

    const msgAntiga = editSnipedmsg.msgAntiga.length > 1024
      ? editSnipedmsg.msgAntiga.slice(0, 1021) + "..."
      : editSnipedmsg.msgAntiga;
    const msgNova = editSnipedmsg.msgNova.length > 1024
      ? editSnipedmsg.msgNova.slice(0, 1021) + "..."
      : editSnipedmsg.msgNova;
    ...
    const Embed = new MessageEmbed()
      ...
      .setAuthor({
        name: `${editSnipedmsg.autor.username} editou:`,
        iconURL: editSnipedmsg.autor.displayAvatarURL({ dynamic: true, size: 32 })
      })
      .addFields(
