...
module.exports = {
  ...
  async executar(iCmd, opcoes) {
    ...
    const amorLevel = amorIndex === 10
      ? "💗".repeat(10)
      : "❤️".repeat(amorIndex) + "💔".repeat(10 - amorIndex);

    const Embed = new MessageEmbed()
      .setColor(client.defs.corEmbed.normal)
      .setDescription(
        (
          usuario1.id === usuario2.id
            ? `${usuario1.toString()} se ama `
            : `${usuario1.toString()} ama ${usuario2.toString()} `
        )
        + `esse tanto:\n**${amorPorce}%** │ ${amorLevel}`
      )
  ...
  async executarCtx(iCtx) {
    const opcoes = {
      subComando: "voce",
      voce: { usuario: { usuario: iCtx.targetUser } },
      outros: {
        usuario1: null,
        usuario2: null,
      }
