...
module.exports = {
  ...
  async executar(iCmd, opcoes) {
    ...
    if (!bichinho && opcoes.numero) {
      return client.responder(
        iCmd,
        "bloqueado",
        "Bichinho escolhido não encontrado",
        `Escolha um bichinho entre 1 e ${bichinhos.length}`
      );
    }
    if (!bichinho && !opcoes.numero) {
      return client.responder(
    ...
    const imagem = bichinho.attachments.first()
      ? bichinho.attachments.first().proxyURL
      : bichinho.content.match(filtro)[0];
    ...
    const Embed = new MessageEmbed()
      ...
      .setTitle(
        (opcoes.numero ? "🐶 Bichinho" : "🐶 Bichinho aleatório")
        + ` (${escolhido + 1}/${bichinhos.length})`
      )
      .setTimestamp(bichinho.createdAt.toISOString())
      .setFooter({
        text: bichinho.author.tag,
        iconURL: bichinho.author.avatarURL({ dynamic: true, size: 32 })
      });
