...
module.exports = {
  ...
  async executar(iCmd, opcoes) {
    ...
    const EmbedPergunta = new MessageEmbed()
      ...
      .setAuthor({
        name: iCmd.member.displayName,
        iconURL: iCmd.member.displayAvatarURL({ dynamic: true, size: 32 })
      })
    ...
    const EmbedResposta = new MessageEmbed()
      ...
      .setAuthor({
        name: iCmd.guild.me.displayName,
        iconURL: iCmd.guild.me.displayAvatarURL({ dynamic: true, size: 32 })
      })
  ...
  async executarCtx(iCtx) {
    if (!iCtx.targetMessage.content) {
      return client.responder(
        iCtx,
        "bloqueado",
        "Nenhum texto encontrado",
        "Selecione uma mensagem que tenha um texto"
      );
    }

    const opcoes = { pergunta: iCtx.targetMessage.content };
