...
module.exports = {
  ...
  async executar(iCmd, opcoes) {
    if (!/^[a-zA-Zà-úÀ-Ú]{1,100}$/.test(opcoes.para)) {
      return client.responder(
        iCmd,
        "bloqueado",
        "Tipo de sugestão inválido",
        "O tipo só pode conter letras com o máximo de 100 caracteres"
      );
    }
    if (!/^.{1,4000}$/.test(opcoes.sugestao)) {
      return client.responder(
        iCmd,
        "bloqueado",
        "Sugestão inválida",
        "Sua sugestão só pode conter o máximo de 4000 caracteres"
      );
    }
