  ...
  async executar(aniversariantes) {
    try {
      ...
      if (!canal.permissionsFor(client.user).has("SEND_MESSAGES")) {
        return client.log("aviso", "A mensagem de aniversário não foi enviada por falta de permissões");
      }
      ...
      if (!canal.permissionsFor(client.user).has("MANAGE_CHANNELS")) {
        return client.log("aviso", "Não consigo alterar o tópico do canal por falta de permissões");
      }
