...
module.exports = {
  ...
  async executar(filaMusicas, musica) {
    ...
    const link = new MessageButton().setLabel("Ir para música").setStyle("LINK").setURL(musica.url);
    const Embed = new MessageEmbed()
      .setColor(client.defs.corEmbed.normal)
      .setTitle("▶️ Tocando música")
      .setDescription(
        `[${musica.uploader.name}](${musica.uploader.url} 'Ir para autor') - ${musica.name}`
      )
