    ...
    async executar(interacao) {
        try {
            ...
            if (interacao.isMessageContextMenu() || interacao.isUserContextMenu()) {
                ...
                const excTempo = new Date();
                client.log("comando", `Comando contextual "${iCmd.commandName}" usando em #${formatarCanal(iCmd.channel)} por @${iCmd.user.tag}`);

                const comando = client.comandos.find(c => c.nomeCtx === iCmd.commandName);
                if (!comando) return client.log("critico", `Comando contextual "${iCmd.commandName}" não encontrado`);
                ...
                try {
                    ...
                    client.log("comando", `Comando contextual "${iCmd.commandName}" foi respondido em ${(new Date().getTime() - excTempo.getTime())}ms`);
                } catch (err) {
                    client.log("erro", err.stack);
            ...
            if (interacao.isAutocomplete()) {
                ...
                if (!comando) return client.log("critico", `Comando "${interacao.commandName}" não encontrado`);
