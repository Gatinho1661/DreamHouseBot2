...
module.exports = {
  ...
  async executar(membro) {
    try {
      ...
      if (!config) return client.log("servidor", "Config de chegada não encontrada", "erro");
      if (!config.msg) return client.log("servidor", "Mensagem de chegada não encontrada", "erro");
      if (!config.gif) return client.log("servidor", "Gif de chegada não encontrada", "erro");
      if (!config.canalID) return client.log("servidor", "Canal de chegada não encontrada", "erro");

      const canal = await client.channels.fetch(config.canalID);
      if (!canal) return client.log("servidor", "Canal de chegada não encontrada", "erro");
      ...
      membro.roles.add(cargos, "Devolvendo cargos antigos").then(() => {
        ...
        const erroEmbed = new MessageEmbed()
          ...
          .setDescription(
            "não consegui adicionar seus cargos de volta,"
            + " fale com algum superior para adicionar seus cargos de volta"
          );
